...
public class ClienteDAO {
	public String criar(Cliente cliente) {
		String sqlInsert = "INSERT INTO cliente(CPF, nomeCliente, telefoneCliente, emailCliente, senhaCliente, "
				+ "enderecoRuaCliente, enderecoNumeroCliente, enderecoCEPCliente) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
		...
		try (Connection conn = ConnectionFactory.obtemConexao();
			...
			stm.setString(6, cliente.getEnderecoRua());
			stm.setString(7, cliente.getEnderecoNumero());
			stm.setString(8, cliente.getEnderecoCEP());
			stm.execute();
	...
	public void atualizar(Cliente cliente) {
		String sqlUpdate = "UPDATE cliente SET nomeCliente=?, telefoneCliente=?, emailCliente=?, senhaCliente=?, "
				+ "enderecoRuaCliente=?, enderecoNumeroCliente=?, enderecoCEPCliente=? WHERE CPF=?";
		// usando o try with resources do Java 7, que fecha o que abriu
		try (Connection conn = ConnectionFactory.obtemConexao();
			...
			stm.setString(5, cliente.getEnderecoRua());
			stm.setString(6, cliente.getEnderecoNumero());
			stm.setString(7, cliente.getEnderecoCEP());
	...
	public Cliente carregar(String CPF) {
		...
		String sqlSelect = "SELECT nomeCliente, telefoneCliente, emailCliente, senhaCliente, "
				+ "enderecoRuaCliente, enderecoNumeroCliente, enderecoCEPCliente FROM cliente WHERE cliente.CPF = ?";
		// usando o try with resources do Java 7, que fecha o que abriu
		try (Connection conn = ConnectionFactory.obtemConexao();
			...
			try (ResultSet rs = stm.executeQuery();) {
				if (rs.next()) {
					...
					cliente.setEnderecoRua(rs.getString("enderecoRuaCliente"));
					cliente.setEnderecoNumero(rs.getString("enderecoNumeroCliente"));
					cliente.setEnderecoCEP(rs.getString("enderecoCEPCliente"));
				} else {
					cliente.setCPF(null);
